...
public class MyWallet {
	...
	public void start() {
	    ...
	    walletAppKit.wallet().addCoinsReceivedEventListener(
	    		  (wallet, tx, prevBalance, newBalance) -> {
	    		    ...
	    		    //“run this code when the transaction has at least one confirmation”
	...
	public int send(String value, String to) {
	    try {
	        Address toAddress = 
	        ...
	        Wallet.SendResult sendResult = 
	          ...
	          sendRequest); //4 - transactionbroadcaster, address to send coins to, amount of money
	        				//If the wallet doesn’t contain enough money, the sendCoins method will throw an exception containing some info about how much money was missing.

	        //sendResult is object containing both the transaction that was created
